    ...
    $.preLoadImages = function(images, callback) {
        ...
        for (var i = args_len; i--;) {
            ...
            cacheImage.load(function () {
                loadcount++;
                if(loadcount == args_len && callback) {
                    callback(cache);
                }
            });
            cacheImage.attr('src', images[i]);
            if(cacheImage.complete) {
                $(cacheImage).trigger('load');
            }
 ...
 *
 *	Added start method, so it can continue again by Mark van Duijker
 *
 */
...
$(document).ready(function() {
    // menu fix
    $('#block-menu-primary-links ul.menu').hover(
        function() {
    ...
    // fix all image links (borders)
    $('a img').parents('a').addClass('hasImage');

    // open external links in new browser window
    $('a[rel=external]').attr('target', '_blank');

    // frontpage caroussel
    fp_container = $('#frontpage_items_container');

    if(fp_container.length) {
        var interval = 6000;
        var fp_items = $('#frontpage_items');
        var list = $('.item-list li', fp_items);
        var previous = null;
        var imagesloaded = false;

        // preload images
        var preloads = [];
        $('.field-content-field-fp-image-fid', list).each(function () {
            preloads.push($(this).text());
        });
        $.preLoadImages(preloads, function() {
            imagesloaded = true;
        });

        fp_items.children('.preview-area').css('z-index', 2);
        fp_items.children('.item-list').css('z-index', 3);

        var setActive = function(el, prev) {
            if(!imagesloaded) return;
            $('.active', el.parentNode).removeClass('active');
            $(el).addClass('active');
            //fp_items.css('backgroundImage', "url('" + $('.field-content-field-fp-image-fid', el).text() + "')");
            fp_items.css('backgroundImage', "");
            var src = $('.field-content-field-fp-image-fid', el).text();
            var img = null;
            $(fp_items).children('img').each(function (i, im) {
                if($(im).attr('src') == src) {
                    img = $(im);
                }
            });
            ...
            $(fp_items).children('img').hide();
            img.fadeIn();
        };
        var disableActive = function(el) {
            //$(el).removeClass('active');
        }

        // set caroussel
        var timer = new $.timer(interval, function() {
            var active = $('.active', $(list.parentNode));
            // if active found, find next one in list
            if(active) {
